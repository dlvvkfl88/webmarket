...
app.get('/super_delegationList', session_exists, async function (req, res) {
	...
	var sql = 'SELECT distinct(b.sNameKor) org, b.sDepartmentCode dptCode,a.sPortal portal, a.sEmail email, REPLACE(a.sTel, SUBSTRING(a.sTel, LENGTH(a.sTel)-3, 4), "****") tel, a.sNumber employeeNum, a.sName name, a.nPermission permission, a.nPermission_reusable permission_reusable, a.nPermission_personalchk permission_personalchk, a.nPermission_vr permission_vr ';
	...
	for (var idx = 0; idx < res_org.length; idx++) {
		...
		var org_name = res_org[idx].name;
		var org_mail = res_org[idx].email;
		var org_portal = res_org[idx].portal;

		var sub_name = org_name.substring(1,org_name.length - 1);

		var mask_chat ='';
		
		for(var cnt = 0; cnt < sub_name.length; cnt++)
		{
			mask_chat += '*';
		}

		var change_name = org_name.replace(sub_name, mask_chat);

		res_org[idx].name = change_name;

		var sub_portal = org_portal.substring(1,org_portal.length - 1);

		mask_chat ='';

		for(var cnt = 0; cnt < sub_portal.length; cnt++)
		{
			mask_chat += '*';
		}

		var change_portal = org_portal.replace(sub_portal, mask_chat);

		res_org[idx].portal = change_portal;

		if(org_mail != null)
		{
			var at_index = org_mail.indexOf('@');
			var sub_mail = org_mail.substring(1,at_index - 1);

			mask_chat ='';

			for(var cnt = 0; cnt < sub_mail.length; cnt++)
			{
				mask_chat += '*';
			}

			var change_mail = org_mail.replace(sub_mail, mask_chat);

			res_org[idx].email = change_mail;
		}
	}
...
app.get('/reusable_market_new', session_exists, async function (req, res) {
	...
	// 기본 쿼리문
	let baseQuery = 'SELECT a.*,b.sImgPath,b.sImgBin,ap.nReusableNo AS ap_nReusableNo, ap.nApprovalState,ap.nApprovalType FROM carbonKU.tblReusable a JOIN carbonKU.tblReusableImg b ON a.nReusableNo = b.nReusableIdx LEFT JOIN carbonKU.tblApproval ap ON a.nReusableNo = ap.nReusableNo WHERE (a.nApplicantState < 2 OR a.nReusableState < 2)';
	// 검색 조건 추가
    ...
    FROM carbonKU.tblReusable a 
    JOIN carbonKU.tblReusableImg b ON a.nReusableNo = b.nReusableIdx 
    LEFT JOIN carbonKU.tblApproval ap ON a.nReusableNo = ap.nReusableNo 
    WHERE (a.nApplicantState < 2 OR a.nReusableState < 2)`;
...
app.get('/registration_list_new', session_exists, async function (req, res) {
	try {
		...
		// 아이템 조회
		var sql = `SELECT a.*, b.sImgPath, b.sImgBin, ap.nReusableNo AS approvalReusableNo, ap.nApprovalState, ap.nApprovalType FROM carbonKU.tblReusable a JOIN carbonKU.tblReusableImg b ON a.nReusableNo = b.nReusableIdx LEFT JOIN carbonKU.tblApproval ap ON a.nReusableNo = ap.nReusableNo WHERE a.sReusableUserNo = ? ORDER BY a.dFixtureDate DESC LIMIT ? OFFSET ?`;
...
app.post('/return_request', async function (req, res) {
	...
	for (var fix_id = 0; fix_id < fix_list.length; fix_id++) {
		...
		// 이제 하드코딩 대신 요청받은 approver_list의 데이터에서 sNumber를 사용
		for (var approver_idx = 0; approver_idx < approver_list.length; approver_idx++) {
			// 요청받은 값의 employeeNum을 사용합니다.
			let customSNumber = approver_list[approver_idx].employeeNum;
	...
	// 결재자에게 각각 한 통씩 이메일 발송 (총 approver_list에 있는 3명의 이메일 사용)
	for (var approver_idx = 0; approver_idx < approver_list.length; approver_idx++) {
		// 해당 결재자의 이메일 주소 사용
		let mailOptions = {
			from: 'kbw3672@naver.com',
			to: approver_list[approver_idx].email,
	...
	try {
		...
		const sql = `
			SELECT 
				...
				b.sImgPath, 
				b.sImgBin, 
				ap.nReusableNo AS approvalReusableNo, 
				ap.nApprovalState, 
				ap.nApprovalType 
			FROM carbonKU.tblReusable a 
			-- 이미지 테이블과 조인 시 올바른 컬럼(nReusableIdx) 사용
			JOIN carbonKU.tblReusableImg b ON a.nReusableNo = b.nReusableIdx
			LEFT JOIN carbonKU.tblApproval ap ON a.nReusableNo = ap.nReusableNo 
			WHERE a.nReusableNo = ? 
			ORDER BY a.dFixtureDate DESC
		...
		if (result.length > 0) {
			...
			res.json({
				...
				// fixturePrice 컬럼명이 실제 DB 컬럼명과 다를 수 있으니 수정 필요
				fixturePrice: item.sFixturePrice,
...
async function login_process(userid, userpw, msg_data, request, response) {
	...
	for (var i = 0; i < datas.length; i++) {
		...
		else if (key_data[0] == "sGid") {
			if(key_data[1] == "P1" || key_data[1] == "P2" || key_data[1] == "P3")
			{
...
app.get('/equipment_inquiry', session_exists, async function (req, res) {
	try {
		...
		// authority_list 조회 추가
		const authorityQuery = "SELECT * FROM tblAuthority WHERE sAuthorityNum=?";
		const authority_list = await directQuery(authorityQuery, [user_id]);
		...
		// 물품 목록 조회
		var sql = `SELECT sFixtureName,sFixtureNo,sFixturePrice,dFixtureDate,sFixtureType,sFixtureModel 
                  FROM tblFixture ${whereClause} 
                  ORDER BY dFixtureDate ASC LIMIT ? OFFSET ?`;
		var pinfos = await directQuery(sql, [...params, itemsPerPage, nStart * itemsPerPage]);
		...
		res.render('equipment_inquiry', {
			...
			searchKeyword,
			session: req.session,
			userid: user_id,  // userid는 한 번만 전달
			user_email,
			user_phone,
			idx: req.session.user.name,
			uname: req.session.user.name,
...
/*
const options = {
    key: fs.readFileSync('./certs/star.korea.ac.kr.key'),
    cert: fs.readFileSync('./certs/star.korea.ac.kr.crt'),
...
var SSLport = process.env.PORT || 33000;

// 운영 서버
//var port = process.env.PORT || 31000;
//var SSLport = process.env.PORT || 31000;
//var host = "0.0.0.0";
...
// HTTPS 서버
const httpsServer = https.createServer(options, app).listen(SSLport, function(){
	console.log('Order API is runnning at ' + SSLport);
});
