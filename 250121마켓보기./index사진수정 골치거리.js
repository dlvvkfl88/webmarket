...
app.get('/home', session_exists, async function (req, res) {
	...
	const recentItemsQuery = `
        ...
        FROM tblReusable a 
        JOIN tblReusableImg b ON a.nReusableNo = b.nReusableIdx 
        WHERE a.nReusableState = 1 AND a.nApplicantState = 0 
...
app.post('/reusable/update-progress', upload.single('file'), async (req, res) => {
	let pool;
	try {
		console.log('Received request body:', {
			reusableNo: req.body.reusableNo,
			title: req.body.title,
			content: req.body.content,
			mainFileName: req.body.mainFileName,
			base64DataLength: req.body.base64Data ? req.body.base64Data.length : 0
		});
		...
		const mainFileName = req.body.mainFileName;

		if (!reusableNo) {
			throw new Error('Reusable number is required');
		}

		// base64Data가 너무 길 경우 잘라내기
		if (base64Data && base64Data.length > 16777215) { // MySQL MEDIUMTEXT 최대 길이
			base64Data = base64Data.substring(0, 16777215);
		}
		...
		// tblReusable 업데이트
		const updateReusableQuery = 'UPDATE tblReusable SET sReusableName = ?, sReusableMemo = ? WHERE nReusableNo = ?';
		await pool.query(updateReusableQuery, [title || '', content || '', reusableNo]);

		// 이미지 업데이트 로직
		if (base64Data && mainFileName) {
			// 이미지 존재 여부 확인
			...
			if (checkResult[0].count > 0) {
				...
				await pool.query(
					...
					[base64Data, mainFileName, reusableNo]
				);
			} else {
				// 이미지가 없으면 INSERT
				await pool.query(
					'INSERT INTO tblReusableImg (nReusableIdx, sImgBin, sImgPath) VALUES (?, ?, ?)',
					[reusableNo, base64Data, mainFileName]
				);
			}
		}
...
app.get('/approval_applicationPopup03', session_exists, async function (req, res) {
...
app.post('/return_request', async function (req, res) {
	...
	for (var fix_id = 0; fix_id < fix_list.length; fix_id++) {
		sql = 'INSERT INTO tblApproval(nReusableNo,sFixtureNo,sApprovalTitle,dApprovalDate,sApprovalReason,sFixtureName,sBuild,sRoomNo,sFixturePrice,nReusableRank,nApprovalState,sUserName,sUserOrg,sUserPortal,nApprovalType) ';
		sql += 'VALUES(?,?,?,NOW(),?,?,?,?,?,?,1,?,?,?,2);';

		var approval_ret = await directQuery(sql, [fix_list[fix_id].nReusableNo, fix_list[fix_id].sFixtureNo, approval_title, approval_memo, fix_list[fix_id].sFixtureName, fix_list[fix_id].sFixtureBuildName, fix_list[fix_id].sFixtureRoomNo, fix_list[fix_id].sFixturePrice, fix_list[fix_id].nReusableRank, user_name, user_org, user_info[0].sPortal]);
		...
		for (var approver_idx = 0; approver_idx < approver_list.length; approver_idx++) {
			...
			if (approver_idx == 0) {
				...
				html_data += "불용 리스트<br>" + fix_list[fix_id].sFixtureNo + " " + fix_list[fix_id].sFixtureName + " " + fix_list[fix_id].sFixtureBuildName + " " + fix_list[fix_id].sFixtureRoomNo + " " + fix_list[fix_id].sFixturePrice;
...
app.post('/deleteReturnReusableInfo', async function (req, res) {

	var resu_list = req.body.resuList;

	var sql = "DELETE FROM tblReusable WHERE nReusableNo IN (?);";
	var resu_del = await directQuery(sql, [resu_list]);
...
app.get('/approval_box_new', session_exists, async function (req, res) {
	...
	sql = "SELECT * FROM tblApprover where sPortal = ?;";
	var appprover_list = await directQuery(sql, [req.session.user.portalid]);

	//sql = "SELECT * FROM tblApproval where nApprovalType = 2;";
	//var approval_list = await directQuery(sql);

	var approval_list_process = [];
	var approval_list_comp = [];
	...
	for (var idx = 0; idx < appprover_list.length; idx++) {
		var approval_info = await directQuery(sql, [appprover_list[idx].nApprIdx]);

		if (approval_info.length != 0) {
			approval_list_process.push(approval_info[0]);
		}
	}
	...
	for (var idx = 0; idx < appprover_list.length; idx++) {
		...
		if (approval_info.length != 0) {
			approval_list_comp.push(approval_info[0]);
		}
	}
	...
	*/
	...
	res.render("approval_box_new", {
		idx: req.session.user.name, uname: req.session.user.name, userid: req.session.user.userid, permission: req.session.user.permission
		, stdtype: req.session.user.stdtype, authority_list: authority_list, sel_authority: req.session.user.selectAuithority, approval_list_process: approval_list_process, approval_list_comp: approval_list_comp
	});
});
...
app.get('/transactions_list_new', session_exists, async function (req, res) {
	...
	res.render("transactions_list_new", { idx: req.session.user.name, uname: req.session.user.name, userid: req.session.user.userid, permission: req.session.user.permission, stdtype: req.session.user.stdtype, authority_list: authority_list, sel_authority: req.session.user.selectAuithority });
});
...
app.post('/approvalDetail', async function (req, res) {
	...
	//console.log("approval_info",approval_info);

	sql = "SELECT * FROM tblReusable WHERE sFixtureNo=?";
