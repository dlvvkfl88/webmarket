...
app.get('/reusable_market_new', session_exists, async function (req, res) {
	...
	// 기본 쿼리문
	let baseQuery = 'SELECT a.*,b.sImgPath,b.sImgBin FROM tblReusable a, tblReusableImg b WHERE a.nReusableNo=b.nReusableIdx AND (a.nApplicantState < 2 OR a.nReusableState < 2)';
	// 검색 조건 추가
		...
		for (var approver_idx = 0; approver_idx < approver_list.length; approver_idx++) {
			...
			if (approver_idx == 0) {
				...
				let mailOptions = {
					...
					to: "xhdlsql12@kounosoft.com", // 수신할 이메일
...
app.get('/approval_box_new', session_exists, async function (req, res) {
	...
	// 권한 목록을 순회하면서 각 권한에 대한 사용자 이름 설정
	for (var idx = 0; idx < authority_list.length; idx++) {
		// 특정 ID에 대한 하드코딩된 이름 매핑
		if (authority_list[idx].sStdId == "210597") {
			authority_list[idx].sName = "원격교육센터 02";
		}
		else if (authority_list[idx].sStdId == "597210") {
			...
			// 조회된 결과가 있으면 이름 설정, 없으면 "데이터 없음" 설정
			if (result.length > 0) {
				authority_list[idx].sName = result[0].sName;
			}
			else {
	...
	// 진행중인 승인 목록을 조회하는 SQL 쿼리
	sql = "SELECT * FROM tblApproval where nApprovalType=2 and nApprovalState!=4;";

	// 각 승인자에 대한 진행중인 승인 정보 조회
	/*
	...
	*/
	...
	approval_list_process = await directQuery(sql);

	// 완료된 승인 목록을 조회하는 SQL 쿼리
	sql = "SELECT * FROM tblApproval where nApprovalType=2 and nApprovalState=4;";

	// 각 승인자에 대한 완료된 승인 정보 조회
	/*
	for (var idx = 0; idx < appprover_list.length; idx++) {
		var approval_info = await directQuery(sql, [appprover_list[idx].nApprIdx]);
	...
	*/
	...
	approval_list_comp = await directQuery(sql);
...
// 소개 인사말 페이지
app.get(['/intro_greeting'], (req, res) => {
	res.render('intro_greeting');
});

// 소개 서비스 소개 페이지
app.get(['/intro_introduction'], (req, res) => {
	res.render('intro_introduction');
});

// 소개 이용방법 페이지
app.get(['/intro_how'], (req, res) => {
	res.render('intro_how');
});

// 소개 esg 페이지
app.get(['/intro_ESG'], (req, res) => {
	res.render('intro_ESG');
});

// 소개 기술지원 페이지
app.get(['/intro_support'], (req, res) => {
	res.render('intro_support');
});

// 소개 UI 자료 페이지
app.get(['/intro_UIdown'], (req, res) => {
...
// 비품 실사 물품 실사 페이지
app.get(['/equipment_idiligence'], (req, res) => {
	res.render('equipment_idiligence');
});
// 비품 실사 실사 현황 페이지
app.get(['/equipment_current'], (req, res) => {
	res.render('equipment_current');
});

// 부서 조회 팝업
app.get(['/department_list_Popup'], (req, res) => {
	res.render('department_list_Popup');
});

// 권한 위임
app.get(['/right_authority'], (req, res) => {
